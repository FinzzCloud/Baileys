//=======================================================//
// builder.js  --  Button / ButtonV2 / Carousel / AIRich / Toolkit
// Porting dari baileys-corp (CJS) ke ESM untuk Baileys-finzzcloud.
// Dependensi ke socket hanya: relayMessage + waUploadToServer.
//=======================================================//
import crypto from "crypto";
import { PassThrough, Readable } from "stream";
import { generateMessageIDV2, generateWAMessageFromContent, prepareWAMessageMedia } from "../Utils/index.js";
//=======================================================//
// sharp & fluent-ffmpeg HANYA dipakai 2 helper opsional (Toolkit.resize dan ambil frame video).
// Dimuat malas (lazy) agar bot yang tidak memakainya tidak wajib memasang keduanya.
const lazy = (name) => {
  let mod;
  return async () => {
    if (!mod) {
      try { mod = (await import(name)).default ?? (await import(name)); }
      catch { throw new Error(`Modul "${name}" belum terpasang. Jalankan: npm i ${name}`); }
    }
    return mod;
  };
};
const getSharp = lazy("sharp");
const getFfmpeg = lazy("fluent-ffmpeg");
//=======================================================//
...
class Toolkit {
	...
	static async getMp4Preview(videoBuffer, { time, result = 'buffer', resize = true, width = 300, height = 300, silent = true } = {}) {
		let ffmpeg;
		try { ffmpeg = await getFfmpeg(); }
		catch (e) { if (silent) return result === 'base64' ? '' : Buffer.alloc(0); throw e; }
...
//=======================================================//
export const BUILDER_VERSION = "port-1";
